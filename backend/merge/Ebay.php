...
class Ebay{
	...
	private string $uri_finding = "https://perso.univ-lemans.fr/~acorbi/services/search/FindingService/v1/";
	//private $api_endpoint = "http://api.ebay.com/ws/api.dll" ;
	private string $api_endpoint = "https://perso.univ-lemans.fr/~acorbi/ws/api.dll/" ;
     ...
     * @return Ebay
     */
    public function setCategory(string $category = "all"): Ebay
	{
     ...
     * @return id
     */
    public function getParentCategoryIdByName(string $category = "all"): int
	{
		...
		if($category == "Animalerie" && $this->globalId == "EBAY-FR") {
			$categoryId = 1281 ; // Livres EBAY-FR
		} 
		if($category == "Auto" && $this->globalId == "EBAY-FR") {
			$categoryId = 131090 ; // Livres EBAY-FR
		} 

		if($category == "Loisirs" && $this->globalId == "EBAY-FR") {
			$categoryId = 14339 ; // Livres EBAY-FR
		} 

		
		if($category == "Bricolage" && $this->globalId == "EBAY-FR") {
			$categoryId = 3187 ; // Livres EBAY-FR
		} 

		if($category == "Informatique" && $this->globalId == "EBAY-FR") {
			$categoryId = 58058 ; // Livres EBAY-FR
		} 

		if($category == "Jeux-video" && $this->globalId == "EBAY-FR") {
			$categoryId = 1249 ; // Livres EBAY-FR
		} 

		if($category == "Sante-hygiene" && $this->globalId == "EBAY-FR") {
			$categoryId = 67588 ; // Livres EBAY-FR
		} 

			if($category == "Bijoux-montres" && $this->globalId == "EBAY-FR") {
			$categoryId = 281 ; // Livres EBAY-FR
		} 

		if($category == "Dvd-cinema" && $this->globalId == "EBAY-FR") {
			$categoryId = 11232 ; // Livres EBAY-FR
		} 


		if ($category == "all" && $this->globalId == "EBAY-FR") {
			$categoryId = 1; // Livres EBAY-FR
		} 
    ...
    public function getParentCategoryIdById(int|string $id): int
	{
		...
		$tabIdCategories[1281] = 1281; //Animalerie
		$tabIdCategories[131090] = 131090; //Auto
		$tabIdCategories[14339] = 14339; //Loisirs
		$tabIdCategories[3187] = 3187; //Bricolage
		$tabIdCategories[1249] = 1249; //Jeux vidéo et console
		$tabIdCategories[58058] = 58058; //Informatique
		$tabIdCategories[67588] = 67588; //Santé et hygiènes
		$tabIdCategories[281] = 281; //Bijoux et montres
		$tabIdCategories[11232] = 11232; //Dvd et cinéma
